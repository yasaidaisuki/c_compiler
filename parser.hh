...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // statement
      // expression
      char dummy3[sizeof (std::unique_ptr<ASTNode>)];

      // block
      char dummy4[sizeof (std::unique_ptr<Block>)];

      // program
      char dummy5[sizeof (std::unique_ptr<TopLevel>)];

      // parameter_list
      char dummy6[sizeof (std::vector<std::string>)];

      // statement_list
      char dummy7[sizeof (std::vector<std::unique_ptr<ASTNode>>)];
    };
    ...
    OPEN_BRACE = 3,                // OPEN_BRACE
    CLOSE_BRACE = 4,               // CLOSE_BRACE
    OPEN_PAR = 5,                  // OPEN_PAR
    CLOSE_PAR = 6,                 // CLOSE_PAR
    IF = 7,                        // IF
    WHILE = 8,                     // WHILE
    LESS_OR_EQUAL = 9,             // LESS_OR_EQUAL
    KEYWORD_INT = 10,              // KEYWORD_INT
    RETURN = 11,                   // RETURN
    SEMI_COL = 12,                 // SEMI_COL
    EQUAL = 13,                    // EQUAL
    COMMA = 14,                    // COMMA
    ADD = 15,                      // ADD
    SUB = 16,                      // SUB
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_OPEN_BRACE = 3,                        // OPEN_BRACE
        S_CLOSE_BRACE = 4,                       // CLOSE_BRACE
        S_OPEN_PAR = 5,                          // OPEN_PAR
        S_CLOSE_PAR = 6,                         // CLOSE_PAR
        S_IF = 7,                                // IF
        S_WHILE = 8,                             // WHILE
        S_LESS_OR_EQUAL = 9,                     // LESS_OR_EQUAL
        S_KEYWORD_INT = 10,                      // KEYWORD_INT
        S_RETURN = 11,                           // RETURN
        S_SEMI_COL = 12,                         // SEMI_COL
        S_EQUAL = 13,                            // EQUAL
        S_COMMA = 14,                            // COMMA
        S_ADD = 15,                              // ADD
        S_SUB = 16,                              // SUB
        ...
        S_LESS = 19,                             // LESS
        S_NOT = 20,                              // NOT
        S_NEGATION = 21,                         // NEGATION
        S_INT_LITERAL = 22,                      // INT_LITERAL
        S_IDENTIFIER = 23,                       // IDENTIFIER
        S_YYACCEPT = 24,                         // $accept
        S_program = 25,                          // program
        S_function = 26,                         // function
        S_parameter_list = 27,                   // parameter_list
        S_block = 28,                            // block
        S_statement_list = 29,                   // statement_list
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_program: // program
        value.move< std::unique_ptr<TopLevel> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::unique_ptr<TopLevel>&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::unique_ptr<TopLevel>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_program: // program
        value.template destroy< std::unique_ptr<TopLevel> > ();
        break;
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
...
#endif
      {
        ...
        YY_ASSERT (tok == token::YYEOF
                   || (token::YYerror <= tok && tok <= token::NEGATION));
#endif
      }
#if 201103L <= YY_CPLUSPLUS
    ...
    Parser (Mreow::Scanner& scanner_yyarg, Mreow::Driver& driver_yyarg);
    virtual ~Parser ();
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_OPEN_PAR (location_type l)
      {
        return symbol_type (token::OPEN_PAR, std::move (l));
      }
#else
      static
      symbol_type
      ...
      static
      symbol_type
      make_CLOSE_PAR (location_type l)
      {
        return symbol_type (token::CLOSE_PAR, std::move (l));
      }
#else
      static
      symbol_type
      ...
      static
      symbol_type
      make_KEYWORD_INT (location_type l)
      {
        return symbol_type (token::KEYWORD_INT, std::move (l));
      }
#else
      static
      symbol_type
      make_KEYWORD_INT (const location_type& l)
      {
        return symbol_type (token::KEYWORD_INT, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_SEMI_COL (location_type l)
      {
        return symbol_type (token::SEMI_COL, std::move (l));
      }
#else
      static
      symbol_type
      ...
      {
        return symbol_type (token::NEGATION, std::move (l));
      }
#else
      static
      symbol_type
